  ...
  <div class="app-container question-page">
    ...
    <div class="questions">
      <div class="questions-li" v-for="(item,index) in questionMos" :key="index">
        ...
        <div class="opt">
          <el-radio-group v-if="item.questionType == 1" v-model="item.optionsAnswer[0]">
            <el-radio
          ...
          <el-select v-if="item.questionType == 3" v-model="item.optionsAnswer[0]">
            <el-option
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      let result = await api.getInsureAnswer({
        serviceOrderId: serviceOrderId
      });
      if (result.status == 1) {
        if (result.data) {
          this.questionMos = result.data.insuredQuestionMos;
        } else {
          this.$message({
            type: "error",
            message: "用户没有填写问卷",
